...
is( eval "use unless ($v_minus < \$]), strict => 'subs'; \${'f'} = 12", 12,
    '"use unless" with a false condition, fake pragma');

is( eval "use unless ($v_minus < \$]), strict => 'refs'; \${'f'} = 12", 12,
    '"use unless" with a false condition and a pragma');

is( eval "use unless ($v_plus < \$]), strict => 'subs'; \${'f'} = 12", 12,
...
is( (eval "use unless ($v_plus < \$]), 'open' => IN => ':crlf'; 12" || 0), 12,
